...
/**
 * Build a worker instance with its own session store. The default export is
 * one such instance (what Cloudflare deploys); tests create fresh instances
 * so session state is mechanically isolated instead of shared by session-id
 * convention. Session state remains in-memory per instance — a production
 * deployment would back it with a Durable Object or KV (see README).
 */
export function createWorker({ sessions = createSessionStore() } = {}) {
  async function handleTurn(request, env) {
    ...
    recordTurn(session, {
      ...
      isCrisis: analysis.flags.includes("crisis_language"),
      userMessage: user_message,
