...
- **`console.html`** — admin console: live session monitoring
  (`GET /v1/sessions`), the manual's governance record-keeping
  instruments (AI Use Inventory, Disclosure Review, QA Findings Tracker,
  Release Readiness Checklist) via `/v1/governance/*`, and the AI Trust
  Receipt Workbook's Conformance Decision Protocol (R1–R12) via
  `/v1/trust-receipt/*`. Set the API key field if the worker has `API_KEY`
  configured.
